...
def makeNewTest(n, p, perct):
  ...
  for i in range(maxn+1):
      height.append(round((bottom[i]-top[i]+1)*abs(np.random.normal(perct, 0.05))))
  # print(height)
  for i in range(n):
      for j in range(n):
...
for r in range(0, 105, 5):
  if not os.path.exists(f'newaquarium_10_{r}_test'):
    os.makedirs(f'newaquarium_10_{r}_test')
  for i in range(200):
    f = open(f'newaquarium_10_{r}_test/test{i}.txt', 'w')
    f.write(makeNewTest(10, 0.8, r/100))
    f.close()

# for r in range(0, 105, 5):
#     if not os.path.exists(f'../BPPercentage/newaquarium_10_{r}_result'):
#         os.makedirs(f'../BPPercentage/newaquarium_10_{r}_result')